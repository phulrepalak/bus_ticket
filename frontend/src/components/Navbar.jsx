      ...
      <div className="flex flex-wrap items-center justify-end gap-4 text-slate-700 font-medium">
        ...
        {token && userRole === "user" && (
          <>
            ...
            {/* Activity Link Yahan Se Hata Diya Gaya Hai */}
          </>
        )}
        ...
        {!token && isGuest === "true" && (
          <Link 
            to="/track-booking" 
            className={`hover:text-sky-600 transition-colors ${
              location.pathname === "/track-booking" 
                ? "text-sky-600 font-semibold" 
                : ""
            }`}
          >
        ...
        {token ? (
          ...
          <Link
            to="/login"
            className={`bg-sky-500 hover:bg-sky-600 text-white px-6 py-2 rounded-full transition-all shadow-md ${
